...
const getCustomResources = require('./custom-resource-definitions').getCustomResources;
...
class Client {
    ...
    async loadSpec() {
        const spec = await (new KubernetesRequest({
            requestOptions: this._options.requestOptions,
            headers: {
                'accept-encoding': 'gzip'
            },
            path: '/swagger-2.0.0.json'
        })).execute();

        this.resources = new OpenApi(spec, this._options.requestOptions);
        this.resources.addCustomResources(await getCustomResources(this), this._options.requestOptions);
    }
...
Client.paginate = async (resource) => {
    ...
    while (listResponse.metadata.continue) {
