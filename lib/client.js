...
class Client {
    constructor(options = {}) {
        ...
        if(options.version) {
            ...
            if(options.version) {
                ...
                this.resources = new OpenApi(zlib.gunzipSync(fs.readFileSync(specPath)), this._options.serverConfig);
            }
        }
    }
    ...
    async loadSpec() {
        const spec = await (new KubernetesRequest({
            serverConfig: this._options.serverConfig,
            headers: {
        ...
        this.resources = new OpenApi(spec, this._options.serverConfig);
    }
...
Client.Config = {
    ...
    fromKubeConfig: (options = {}) => {
        ...
        if(user['client-certificate'] && user['client-key']) {
            config.user = {
                clientCert: user['client-certificate'],
