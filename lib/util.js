...
function formatAuthOptions(options) {
    ...
    if(options.user) {
        ...
        if(userOpts.cert && userOpts.key) {
            requestOptions.agent = {
                // todo: sync bad here I think
                cert: fs.readFileSync(userOpts.clientCert),
                key: fs.readFileSync(userOpts.clientKey)
            };
        }
    ...
    if(options.ca) {
        requestOptions.agent.ca = fs.readFileSync(options.ca);
    }

    if(options.insecureTlsSkipVerify) {
        requestOptions.agent.rejectUnauthorized = false;
    }

    if(_isEmpty(requestOptions.agent)) {
...
module.exports.formatAuthOptions = formatAuthOptions;
