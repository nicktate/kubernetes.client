        ...
        for(const group of apiGroupList.groups) {
            ...
            for(const resource of apiResourceList.resources) {
                ...
                if(!initialResources[kind]) {
                    ...
                    // Resources support fetching across all namespaces if:
                    // 1. It is namespaced
                    // 2. It supports the `list` verb
                    initialResources[kind].supportsAllNamespaces = resource.namespaced && resource.verbs.indexOf('list') >= 0;
                }
